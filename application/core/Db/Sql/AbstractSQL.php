...
abstract class AbstractSql
{
    ...
    protected function processExpression(ExpressionInterface $expression, PlatformInterface $platform, DriverInterface $driver = null, $namedParameterPrefix = null)
    {
        ...
        foreach ($parts as $part) {
            ...
            if (!is_array($part)) {
                throw new Exception('Elements returned from getExpressionData() array must be a string or array.');
            }
            ...
            if (isset($paramSpecs[$position]['combinedby'])) {
                ...
                foreach ($paramsForPosition as $multiParamsForPosition) {
                    ...
                    if (!isset($paramSpecs[$position][$ppCount])) {
                        throw new Exception('A number of parameters (' . $ppCount . ') was found that is not supported by this specification');
                    }
    ...
    protected function processSubSelect(Select $subselect, PlatformInterface $platform, DriverInterface $driver = null, ParameterContainer $parameterContainer = null)
    {
        if ($driver) {
            ...
            // call subselect
            $subselect->prepareStatement(new \Core\Db\Adapter($driver, $platform), $stmtContainer);
