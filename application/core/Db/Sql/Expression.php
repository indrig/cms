...
class Expression implements ExpressionInterface
{
    ...
    public function setExpression($expression)
    {
        if (!is_string($expression) || $expression == '') {
            throw new Exception('Supplied expression must be a string.');
        }
    ...
    public function setParameters($parameters)
    {
        if (!is_scalar($parameters) && !is_array($parameters)) {
            throw new Exception('Expression parameters must be a scalar or array.');
        }
    ...
    public function getExpressionData()
    {
        ...
        if ($parametersCount > 0) {
            ...
            if ($count !== $parametersCount) {
                throw new Exception('The number of replacements in the expression does not match the number of parameters');
            }
        }
