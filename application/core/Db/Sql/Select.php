...
use Core\Db\AdapterInterface,
    ...
    Core\Db\Platform\Sql92 as AdapterSql92Platform;
...
class Select extends AbstractSql implements SqlInterface, PreparableSqlInterface
{
    ...
    public function from($table)
    {
        if ($this->tableReadOnly) {
            throw new Exception('Since this object was created with a table and/or schema in the constructor, it is read only.');
        }
    ...
    public function setSpecification($index, $specification)
    {
        if (!method_exists($this, 'process' . $index)) {
            throw new Exception('Not a valid specification name.');
        }
    ...
    protected function processOrder(PlatformInterface $platform, DriverInterface $driver = null, ParameterContainer $parameterContainer = null)
    {
        ...
        foreach ($this->order as $k => $v) {
            if ($v instanceof Expression) {
                /** @var $orderParts \Core\Db\StatementContainer */
