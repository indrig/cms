...
use Iterator,
    Exception,
    Core\Db\Driver\ResultInterface;
    ...
    ResultInterface
{
    ...
    public function buffer()
    {
        if ($this->resource instanceof \mysqli_stmt && $this->isBuffered !== true) {
            if ($this->position > 0) {
                throw new Exception('Cannot buffer a result set that has started iteration.');
            }
    ...
    public function rewind()
    {
        if ($this->position !== 0) {
            if ($this->isBuffered === false) {
                throw new Exception('Unbuffered results cannot be rewound for multiple iterations');
            }
        }
    ...
    {
        if ($this->isBuffered === false) {
            throw new Exception('Row count is not available in unbuffered result sets.');
        }
        return $this->resource->num_rows;
    }
