...
class Adapter implements AdapterInterface, Profiler\ProfilerAwareInterface
{
    ...
    protected function createDriver($parameters)
    {
        ...
        if (!is_string($parameters['driver'])) {
            throw new Exception(__FUNCTION__ . ' expects a "driver" to be a string or instance of DriverInterface');
        }
        ...
        switch ($driverName) {
            ...
            default:
                if ($driverName == 'pdo' || strpos($driverName, 'pdo') === 0) {
                    $driver = new Driver\Pdo\Driver($parameters);
                }
        }
    ...
    protected function createPlatform($parameters)
    {
        if (isset($parameters['platform'])) {
            ...
            throw new Exception('A platform could not be determined from the provided configuration');
        }
        ...
        switch ($platformName) {
            case 'Mysql':
                ...
                $driver = ($this->driver instanceof Driver\Mysqli\Driver || $this->driver instanceof Driver\Pdo\Driver) ? $this->driver : null;
                return new Platform\Mysql($driver);
            case 'SqlServer':
                // PDO is only supported driver for quoting values in this platform
                return new Platform\SqlServer(($this->driver instanceof Driver\Pdo\Driver) ? $this->driver : null);
            case 'Oracle':
                // oracle does not accept a driver as an option, no driver specific quoting available
                return new Platform\Oracle($options);
            case 'Sqlite':
                // PDO is only supported driver for quoting values in this platform
                return new Platform\Sqlite(($this->driver instanceof Driver\Pdo\Driver) ? $this->driver : null);
            case 'Postgresql':
                // pgsql or pdo postgres driver
                $driver = ($this->driver instanceof Driver\Pgsql\Driver || $this->driver instanceof Driver\Pdo\Driver) ? $this->driver : null;
